...
class TempestLogItem(object):
    def __init__(self, req_id, function, req=None, resp=None):
        self.req_id = req_id
    ...
    def from_lines(cls, req_id, info, debug):
        ...
        response = TempestResponse(status=info_match.groupdict()['status'],
                                   ...
                                   body=resp_body)
        item = cls(req_id, function, request, response)
        return item
    ...
    def to_diagram(self):
        return "Client->{host}: {method} {path}\n{host}->Client: {status}".format(method=self.request.method, host=self.request.target, path=self.request.url.path, status=self.response.status)
